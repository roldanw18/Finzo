...
export function Categories() {
  ...
  const monthSpent = useMemo(() => {
    ...
    const slices = expensesByCategory(expenses, categories, range)
    return new Map(slices.map((s) => [s.id, s.value]))
  }, [expenses, categories])
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {categories.map((c, i) => {
          ...
          const spent = monthSpent.get(c.id) ?? 0
          const pct = budget > 0 ? Math.min(100, (spent / budget) * 100) : 0
          const over = budget > 0 && spent > budget
          const near = budget > 0 && !over && pct >= 80
          const barColor = over ? 'rgb(var(--c-expense))' : near ? '#f59e0b' : c.color
          return (
            <motion.button
              key={c.id}
                ...
                <div>
                  <div className="mb-1 flex items-center justify-between text-[11px]">
                    <span className="text-muted">
                      {money(spent, { compact: true })} / {money(budget, { compact: true })}
                    </span>
                    <span
                      className={
